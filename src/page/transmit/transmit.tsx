...
function Transmit_page() {
  ...
  return (
    <div style={{ margin: '0' }}>
      <Card style={{}} title={t('overview')} bordered={false}>
        <Space direction="vertical" style={{ width: '100%' }}>
          {transmitList.length > 0 &&
            rcloneInfo.stats.realSpeed === 0 &&
            rcloneInfo.stats.speed === 0 && (
              <Alert
                style={{ margin: '0.1rem' }}
                type="info"
          ...
          {rcloneInfo.stats.bytes > 0 && (
            <Progress percent={~~((rcloneInfo.stats.bytes / rcloneInfo.stats.totalBytes) * 100)} />
          )}
          <Descriptions
            colon=" :"
            data={[
      ...
      <Card style={{}} title={t('transferring')} bordered={false}>
                  ...
                  <div style={{ width: 'calc(100% - 5rem)', overflow: 'auto' }}>
                    ...
                    <Descriptions
                      ...
                      data={[
                        ...
                        {
                          label: t('size'),
                          value: `${formatSize(item.bytes)}/${formatSize(item.size)}`,
                        },
                        {
                          label: t('source'),
                        ...
                        ...(item.dstFs
                          ? [
                              {
                                label: t('target'),
                                value: (item.dstFs || '').replace(
                                  openlistInfo.markInRclone + ':',
                                  ''
                                ),
                              },
                            ]
