...
const reducer = (state: TaskInfoState, action: Action): TaskInfoState => {
  switch (action.type) {
    ...
    case 'setIntervalDays':
      return {
        ...state,
        run: { ...state.run, time: { ...state.run.time, intervalDays: action.payload } },
      }
    case 'setRunTime':
...
function AddTask_page() {
  ...
  const [taskInfo, dispatch] = useReducer(reducer, {
    ...
    taskType:
      roConfig.options.task.taskType.select[roConfig.options.task.taskType.defIndex] || 'copy',
    source: {
      storageName: storageList && storageList.length > 0 ? storageList[0]?.name || '' : '',
      path: '/',
    },
    target: {
      storageName:
    ...
    run: {
      mode:
        roConfig.options.task.runMode.select[roConfig.options.task.runMode.defIndex] || 'interval',
      time: {
        intervalDays: 1,
        h: 10,
        m: 30,
  ...
  useEffect(() => {
    if (taskInfo.run.mode === 'time') {
      ...
      setTimeMultiplier({
        name: multiplier?.name || 'day',
        value: multiplier?.value || 1,
        multiplicand: 1,
      })
    } else if (taskInfo.run.mode === 'interval') {
      const multiplier =
        roConfig.options.task.intervalMultiplier.select[
          roConfig.options.task.intervalMultiplier.defIndex
        ]
      setTimeMultiplier({
        name: multiplier?.name || 'day',
        value: multiplier?.value || 1,
  ...
  return (
    <div style={{ width: '100%', height: '100%' }}>
      <h2 style={{ fontSize: '1.5rem', marginBottom: '2rem', marginLeft: '1.8rem' }}>
        {!isEditMode ? t('add_task') : t('edit_task')}
      </h2>
              ...
              <Row>
                ...
                <Col flex={'auto'}>
                  <InputNumber
                    ...
                    min={1}
                    max={10000}
                    value={timeMultiplier.multiplicand}
                    precision={0}
                    onChange={value => {
                    ...
                    <Col flex={'1'}>
                      <InputNumber
                        ...
                        onChange={value =>
                          dispatch({
                            type: 'setRunTime',
                            payload: { ...taskInfo.run.time, h: value },
                          } as Action)
                        }
                      />
                    </Col>
                    <Col flex={'1'}>
                      <InputNumber
                        ...
                        onChange={value =>
                          dispatch({
                            type: 'setRunTime',
                            payload: { ...taskInfo.run.time, m: value },
                          } as Action)
                        }
                      />
                    </Col>
                    <Col flex={'1'}>
                      <InputNumber
                        ...
                        onChange={value =>
                          dispatch({
                            type: 'setRunTime',
                            payload: { ...taskInfo.run.time, s: value },
                          } as Action)
                        }
                      />
                    </Col>
          ...
          <Row>
            <Col flex={'7rem'}>
              <Select
                ...
                onChange={value =>
                  dispatch({ type: 'setSourceStorageName', payload: value } as Action)
                }
              >
            ...
            <Row>
              <Col flex={'7rem'}>
                <Select
                  ...
                  placeholder={t('please_select')}
                  onChange={value =>
                    dispatch({ type: 'setTargetStorageName', payload: value } as Action)
                  }
                >
                  {storageList.map(item => (
                    <Select.Option key={item.name} value={item.name}>
      ...
      <div style={{ marginTop: '20px', textAlign: 'right' }}>
        <Space>
          ...
          <Button
            ...
            onClick={async () => {
              if (taskInfo.run.mode === 'time') {
                taskInfo.run.time.intervalDays =
                  timeMultiplier.multiplicand * (timeMultiplier.value || 1)
              } else if (taskInfo.run.mode === 'interval') {
              ...
              if (
                (!isEditMode &&
                  nmConfig.task &&
                  nmConfig.task.findIndex(item => item.name === taskInfo.name) !== -1) ||
                !taskInfo.name
              ) {
                Notification.error({
                  title: t('error'),
                ...
                !taskInfo.source.storageName ||
                !taskInfo.source.path ||
                (taskInfo.taskType !== 'delete' &&
                  (!taskInfo.target.storageName || !taskInfo.target.path))
              ) {
                Notification.error({
                  title: t('error'),
                ...
                taskInfo.taskType !== 'delete' &&
                taskInfo.source.path === taskInfo.target.path &&
                taskInfo.source.storageName === taskInfo.target.storageName
              ) {
                Notification.error({
                  title: t('error'),
