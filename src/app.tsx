...
const HomePage = React.lazy(() =>
  import('./page/home/home').then(module => ({ default: module.Home_page }))
)
const StoragePage = React.lazy(() =>
  import('./page/storage/storage').then(module => ({ default: module.Storage_page }))
)
const AddStoragePage = React.lazy(() =>
  import('./page/storage/add').then(module => ({ default: module.AddStorage_page }))
)
const ExplorerPage = React.lazy(() =>
  import('./page/storage/explorer').then(module => ({ default: module.Explorer_page }))
)
const MountPage = React.lazy(() =>
  import('./page/mount/mount').then(module => ({ default: module.Mount_page }))
)
const AddMountPage = React.lazy(() => import('./page/mount/add'))
const TransmitPage = React.lazy(() =>
  import('./page/transmit/transmit').then(module => ({ default: module.Transmit_page }))
)
const TaskPage = React.lazy(() =>
  import('./page/task/task').then(module => ({ default: module.Task_page }))
)
const AddTaskPage = React.lazy(() =>
  import('./page/task/add').then(module => ({ default: module.AddTask_page }))
)
...
//递归查询对应的路由
function searchRoute(path: string, routes: Routers[]): Routers | null {
...
function mapRouters(routes: Routers[]): JSX.Element {
  return (
    <>
      {routes.map(item => {
        // 添加index作为map方法的第二个参数，用于生成唯一键
        if (item.children && item.children.length > 0) {
          return (
            <React.Fragment key={`${item.path}-group`}>
              {' '}
              {/* 给包含子路由的Fragment添加一个唯一的key */}
  ...
  const routers: Array<Routers> = useMemo(
    () => [
      ...
      },
      {
        ...
        ) /* +(rcloneInfo.stats.transferring? '(' + rcloneInfo.stats.transferring.length + ')': '') */,
        path: '/transmit',
    ...
    hooks.navigate = (path: string) => {
      if (path != location.pathname) {
        location.pathname.includes('add') &&
          Message.warning(t('prompt_for_leaving_the_add_or_edit_page'))
        navigate(path)
      }
    }
      ...
      >
        <Header
          style={{
            ...
            borderBlockEnd: '1px solid var(--color-border-2)',
          }}
        >
          <Row>
            ...
            <Col flex={'5rem'} style={{ textAlign: 'right' }}>
              <Button
                onClick={windowsMini}
                icon={<IconMinus style={{ fontSize: '1.1rem', color: 'var(--color-text-2)' }} />}
                type="text"
                style={{ width: '2.5rem', paddingTop: '0.5rem' }}
              />
              <Button
                onClick={windowsHide}
                icon={<IconClose style={{ fontSize: '1.1rem' }} />}
                type="text"
                status="danger"
                style={{ width: '2.5rem', paddingTop: '0.5rem' }}
              />
